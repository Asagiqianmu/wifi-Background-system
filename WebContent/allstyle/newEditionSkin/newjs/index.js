...
/*	var text = $('.chorsesite').text();
	...
	$('.swicth').click(function(){
		...
		if(str=='swicth d'){
			...
			var siteid = $(".chorsesite").attr("id");
			html = "<span><input type='text' readonly='readonly' onclick=\"WdatePicker({dateFmt:\'yyyy-MM-dd\'})\" class='Wdate' id='startTime'> 至 <input type='text' readonly='readonly' onclick=\"({dateFmt:'yyyy-MM-dd'})\" class=\"Wdate\" id=\"endTime\"><button class='a_btn' id='dayquery'>查询</button></span>";
	    	$('#swicthdm').after(html);
	    	var startTime = new Date((new Date().getTime()-11*24*60*60*1000)).Format('yyyy-MM-dd');
		    var endTime = new Date().Format('yyyy-MM-dd');
		    $("#startTime").val(startTime);
...
/*function canvas(pre,canvasId){//环形图
    ...
    if(canvasId=='perception'){
        ctx.strokeStyle = '#03A1B9';
        round.style.color='#03A1B9';
...
function pie(siteId){//饼图
	...
	$.ajax({
		type : "post",
		url : ctx + "/newEditionSkin/getTypeProportion",
		data : {
			siteId : siteId
		},
		async: false,
		success : function(data) {
			...
			if (data.code == 200) {
				...
				$("#pieChart").text("");
				//{finalBili: 1, finalName: "小时", finalCount: 1}
...
function getSubscriberGrowth(siteId){//折线图
	...
	var	charConfigData =	{
	 ...
	 $.ajax({
			type : "get",
			url : ctx + "/newEditionSkin/getSubscriberGrowth",
			data : {
				siteId : siteId
			},
			success : function(data) {
...
function getBusinessData(siteId){
	 $.ajax({
			type : "post",
			url : ctx+"/newEditionSkin/getAllBusinessData",
			data:{
				siteId :siteId
			},
			success : function(data) {
				...
				if(data.code==200){
					$('.independent > span').text(data.data.ylineNum==null?0:data.data.ylineNum);
					$('.register > span').text(data.data.registerNum==null?0:data.data.registerNum);
					$('.pay > span').text(data.data.payNum==null?0:data.data.payNum);
					$('.yesterday > span').text(data.data.APNum==null?0:data.data.APNum);
...
function getDataStatistics(siteId){
	...
	$.ajax({
		type : "get",
		url : ctx+"/newEditionSkin/getTotalMoneyAndPeopleCount",
		data : {
			 siteId:siteId
		},
		success : function(data) {
...
//			//);
	 $.ajax({
		type : "get",
		url : ctx + "/newEditionSkin/getTwelveDaysBeforeRevenue",
		success : function(data) {
	...
	function Thenetuser(siteId){
			$.ajax({
				...
				success : function(data) {
					data = JSON.parse(data);
					if(data.code==200){
						adata = (data.data.permeateRate*100).toFixed(0);// 用户渗透率 (t*100)+'%';
						bdata = (data.data.tryRegisterRate*100).toFixed(0);// 用户渗透率 (t*100)+'%';
						cdata = (data.data.registerPayRate*100).toFixed(0);// 用户渗透率 (t*100)+'%';
